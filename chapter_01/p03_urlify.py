...
def urlify_pythonic_2(string, length):
    s2 = []
    #Replace spaces starting from length backwards
    for c in range(length-1, -1, -1):
        s2.insert(0, '%20') if string[c] == ' ' else s2.insert(0, string[c])
    # Convert to string
    return ''.join(s2)
...
class Test(unittest.TestCase):
    ...
    testable_functions = [urlify_algo, urlify_pythonic, urlify_pythonic_2]
